...
namespace JHPCNDF
{
    ...
    //  none:        圧縮しない
    template <typename T>
    size_t fwrite(const T* ptr, size_t size, size_t nmemb, const int& key, const float& tolerance, const std::string& enc="normal", const std::string& comp = "gzip", const bool time_measuring = false);
